...
# ============ 配置（支持环境变量覆盖） ============
API_KEY = os.environ.get("GEMINI_API_KEY", "sk-geminixxxxx")
HOST = os.environ.get("GEMINI_HOST", "0.0.0.0")
PORT = int(os.environ.get("GEMINI_PORT", "8000"))
CONFIG_FILE = os.environ.get("GEMINI_CONFIG_FILE", "config_data.json")
# 后台登录账号密码
ADMIN_USERNAME = os.environ.get("GEMINI_ADMIN_USERNAME", "admin")
ADMIN_PASSWORD = os.environ.get("GEMINI_ADMIN_PASSWORD", "admin123")
# Token 自动刷新配置
...
MEDIA_BASE_URL = os.environ.get("GEMINI_MEDIA_BASE_URL", f"http://127.0.0.1:{PORT}")
# ==============================
...
def is_continuation(current_messages: list, last_hash: str) -> bool:
    ...
    特殊情况：如果只有一条用户消息但 client 已有 conversation_id，
    说明 Agent 只发了最新消息（或第二轮起的 tool 结果），也视为延续。
    """
    if not last_hash:
        return False
    
    # 找到所有用户消息（包括 tool role 的消息，Agent 会把工具结果作为 user/tool 发送）
    user_indices = [i for i, m in enumerate(current_messages) 
                    if (m.role if hasattr(m, 'role') else m.get('role', '')) == "user"]
    
    if len(user_indices) <= 1:
        # 只有一条用户消息，检查 client 是否已有会话上下文
        # 如果有 conversation_id，说明是同一会话的后续轮次
        if _client and getattr(_client, 'conversation_id', ''):
            return True
        return False
...
async def chat_completions(request: ChatCompletionRequest, authorization: str = Header(None)):
    ...
    try:
        ...
        # ====== 会话复用优化 ======
        # 判断是否延续上一次对话
        continuing = is_continuation(request.messages, _last_user_messages_hash)
        
        if not continuing:
            # 新对话：reset Gemini 侧上下文
            client.reset()
            print(f"🔄 新对话开始 (共 {len(request.messages)} 条消息)")
        else:
            print(f"➡️ 延续会话 (conv_id={client.conversation_id[:16]}..., 共 {len(request.messages)} 条消息)")
        
        # ====== 智能消息裁剪 ======
        # 提取最后一条 user 消息和最近的 tool 结果
        all_messages = []
        for m in request.messages:
            content = m.content
            if content is None:
                # assistant tool_calls 消息的 content 通常为 null，跳过
                continue
            all_messages.append({"role": m.role, "content": content})
        
        if continuing and len(all_messages) > 1:
            # 延续会话：只发送「最近的 tool 结果 + 最后一条 user 消息」
            # Gemini 侧已有之前的上下文，不需要重复发送历史
            trimmed = []
            
            # 保留 system 消息（如果有的话，始终需要）
            ...
            if last_user_idx >= 0:
                # 收集最后一条 user 消息之前的连续 tool 结果（Agent 工具调用结果）
                tool_context = []
                for i in range(last_user_idx - 1, -1, -1):
                    if all_messages[i]["role"] == "tool":
                        tool_context.insert(0, all_messages[i])
                    elif all_messages[i]["role"] == "assistant":
                        # 保留对应的 assistant 消息（工具调用请求）
                        tool_context.insert(0, all_messages[i])
                    else:
                        break  # 遇到上一轮的 user 消息，停止
                
                # 如果有 tool 结果，将它们转为文本附加到用户消息前
                if tool_context:
                    tool_results_text = "\n".join([
                        f"[工具结果 ({m['role']}): {m['content'][:500]}{'...' if len(str(m['content'])) > 500 else ''}]"
                        for m in tool_context if m["role"] == "tool"
                    ])
                    last_user_content = all_messages[last_user_idx]["content"]
                    if isinstance(last_user_content, str) and tool_results_text:
                        all_messages[last_user_idx]["content"] = tool_results_text + "\n\n" + last_user_content
                
                # 最终只发送: [system(可选)] + [最后一条 user]
                trimmed = system_msgs + [all_messages[last_user_idx]]
            else:
                trimmed = all_messages  # fallback
            
            messages = trimmed
            print(f"   ✂️ 消息裁剪: {len(all_messages)} → {len(messages)} 条")
        else:
            # 新对话或只有单条消息：发全部
